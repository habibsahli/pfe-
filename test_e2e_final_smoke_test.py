...
def test_syntax_service():
    code_file = Path(__file__).parent / "backend" / "app/services/forecasting_service.py")
...
def test_syntax_api():
    code_file = Path(__file__).parent / "backend" / "app/api/forecast.py")
    assert code_file.exists(), "File not found"
    content = code_file.read_text()
    try:
        ast.parse(content)
...
def test_impl_doc():
    doc = Path(__file__).parent / "FEATURE_IMPORTANCE_IMPLEMENTATION.md"
...
def test_quick_start():
    doc = Path(__file__).parent / "FEATURE_IMPORTANCE_QUICK_START.md"
...
def test_final_report():
    doc = Path(__file__).parent / "FEATURE_IMPORTANCE_FINAL_REPORT.md"
...
def test_test_files():
    test_files = [
        Path(__file__).parent / "test_feature_importance.py",
        Path(__file__).parent / "test_feature_importance_validation.py",
    ]
...
def test_backwards_compat():
    code_file = Path(__file__).parent / "backend" / "app/api/training.py")
